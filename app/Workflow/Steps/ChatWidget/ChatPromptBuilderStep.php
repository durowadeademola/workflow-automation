...
class ChatPromptBuilderStep implements StepHandler
{
    public function execute(array $config, WorkflowContext $context): array
    {
        ...
        $workingHours = $context->get('trigger.workingHours');
        $isWithinWorkingHours = $context->get('trigger.isWithinWorkingHours', true);
        $workingHoursInstruction = $workingHours
            ? '- The business\'s working hours are: ' . $workingHours . '. Right now, the business is '
                . ($isWithinWorkingHours ? 'OPEN (within working hours)' : 'CLOSED (outside working hours)')
                . ". If the visitor asks whether you're open, what your hours are, or anything about availability, answer using this real information - never guess or make up different hours. If it's currently outside working hours and the visitor wants to speak with a human, let them know the team is currently offline (mention the hours above) and ask for their name and phone number instead of saying you're connecting them right now."
            : '';
        ...
        $handoffInstruction = "- Hand the visitor off to a real team member if: they explicitly ask to speak with a human, a real person, an agent, or support staff; OR they seem frustrated and unable to get what they need from you; OR - most importantly - you don't actually know the answer to their question even after checking the website knowledge, the business knowledge base, and your own general knowledge. Never guess or invent an answer you're not genuinely confident in just to avoid saying you don't know - handing off is always better than a fabricated answer. When any of this applies, respond warmly letting them know you're connecting them with the team right now, then on its own new line add exactly: HANDOFF:REQUESTED\n"
            ...
            . "- ANOTHER IMPORTANT exception: if the conversation history already shows a completed handoff (e.g. a message saying you're connecting them with the team, or that they're back with the AI assistant), do NOT emit HANDOFF:REQUESTED again just because an older message asked for a human - that request was already handled. Only emit it again if the visitor's CURRENT message is itself a new, clear request to speak with a human (not just continuing the conversation, saying hello, or chit-chat).";
        ...
        $appointmentInstruction = "- If the visitor wants to book an appointment, meeting, or consultation, collect their full name and an exact date and time through natural conversation, asking for anything missing one thing at a time. Today's date is {$today} - use it to resolve relative dates like \"tomorrow\" or \"next Monday\" yourself; never ask the visitor to do that math. Never invent a date/time they didn't actually give you. Once you have BOTH their name AND a specific date and time, confirm the details back to them in your reply, then on its own new line add exactly (with real values, valid JSON, 24-hour time): APPOINTMENT:BOOK {\"name\":\"<full name>\",\"scheduled_at\":\"<YYYY-MM-DD HH:MM:SS>\",\"reason\":\"<brief reason if they gave one, else empty string>\"}\n"
            . "- IMPORTANT exception: if the conversation history already shows their appointment was successfully booked (e.g. a message saying \"You're all set for...\"), do NOT emit APPOINTMENT:BOOK again for that same appointment just because they keep chatting, thank you, or mention it again - it's already booked, so simply acknowledge them normally. Only emit APPOINTMENT:BOOK again if the visitor is clearly asking to book an ADDITIONAL appointment, or explicitly wants to change/reschedule the existing one to a new date or time - in that case treat it as a fresh booking with the new details.";
        ...
        $registerInstruction = "- If the visitor wants to register, sign up, or leave their details (including if they click a \"Register with us\" quick reply), collect their full name, telephone number, AND the reason they're reaching out (what they need help with) through natural conversation, asking for anything missing one thing at a time - the reason is required, not optional, so don't skip asking for it even if they've already given their name and phone number. Email is optional - ask for it too, but don't hold up completing registration if they only give a phone number. Once you have their full name, a phone number, AND their reason for reaching out, confirm the details back to them in your reply, then on its own new line add exactly (with real values, valid JSON): REGISTER:DETAILS {\"name\":\"<full name>\",\"phone\":\"<phone number>\",\"email\":\"<email if given, else empty string>\",\"interest\":\"<the reason they're reaching out>\"}";

        // A general reminder, not tied to any one marker above — added
        // after observing a real reply drop the "LEAD:QUALIFIED" label and
        // emit only the bare JSON, which then leaked straight into what the
        // visitor saw. The label text is what the system actually looks for
        // to save that data; the JSON alone means nothing without it.
        $markerComplianceInstruction = "- CRITICAL: whenever you output one of the special markers above (HANDOFF:REQUESTED, APPOINTMENT:BOOK, LEAD:QUALIFIED, REGISTER:DETAILS), you MUST include its exact literal label text immediately before the JSON, every single time - never output just the bare JSON object on its own without the label in front of it, even if you already stated the same details in plain language earlier in your reply.";
        ...
        if ($hasContext) {
            $systemPrompt = "You are a helpful AI assistant for {$businessName}.\n{$baseKnowledge}\n{$knowledgeBaseBlock}Use the following information from the business website to answer the visitor's question as accurately as possible:\n--- WEBSITE KNOWLEDGE ---\n{$ragContext}\n--- END KNOWLEDGE ---\n"
                ...
                . "{$workingHoursInstruction}\n{$handoffInstruction}\n{$appointmentInstruction}\n{$leadInstruction}\n{$registerInstruction}\n{$markerComplianceInstruction}\n"
